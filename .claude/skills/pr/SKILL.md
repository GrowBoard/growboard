...
This ruleset defines the standard patterns for creating and managing Pull Requests in this repository. **All Pull Requests MUST be created in the GrowBoard/growboard repository.**
...
**Example:**
`amitraikwar/GRW-20/update-jira-skills`

## 4. GitHub MCP Server Integration

When creating a PR using the `github-mcp-server` tool, ensure:

1. The `owner` and `repo` are set to `GrowBoard` and `growboard`.
