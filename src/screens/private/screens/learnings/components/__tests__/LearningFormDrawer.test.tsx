...
describe('LearningFormDrawer component', () => {
  ...
  it('renders drawer header, footer, tabs and form inputs', () => {
    ...
    expect(
      screen.getByPlaceholderText('e.g. Topic Subtitle'),
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText('Type your markdown content here...'),
    ).toBeInTheDocument();
  });
  ...
  it('populates fields when editItem is passed', () => {
    ...
    renderWithProviders(
      <LearningFormDrawer {...defaultProps} editItem={editItem} />,
    );

    expect(screen.getByText('Edit Learning')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('e.g. Topic Title')).toHaveValue(
      'Existing Learning',
    );
    expect(screen.getByPlaceholderText('e.g. Topic Subtitle')).toHaveValue(
      'Existing Subtitle',
    );
    expect(
      screen.getByPlaceholderText('Type your markdown content here...'),
    ).toHaveValue('Detailed markdown learning content');
  ...
  it('submits form values on save click', async () => {
    ...
    await act(async () => {
      ...
      fireEvent.change(contentInput, {
        target: { value: 'This is the core content.' },
      });
    });
